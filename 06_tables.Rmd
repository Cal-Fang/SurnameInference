...
```{r tab1}
nameSample <- str_pad(c("HUANG", "CHANG", "KIM", "SUZUKI", "NGUYEN", "INTHAVONG", "PENAFLOR", "ABDULJALIL"), 
                      12, side="right")
tab1 <- probList %>% 
  ...
  t() %>% 
  as_tibble() %>% 
  mutate(V9 = colnames(probList)[1:20])
...
tab1 %>% 
  ...
  add_header_above(header=c(" "=1, "Illustrative surnames[note]"=8)) %>% 
...
tab3 <- predictSummary %>% 
  ...
  mutate(across(.cols=nameCount:Specificity, .fns=~round(as.numeric(.x), 4)*100))
...
<!--
# Table. Numbers of foreign-born records from corresponding country/area
```{r sample tab}
